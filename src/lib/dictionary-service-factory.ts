...
// Disconnected dictionary service
class DisconnectedDictionaryService implements DictionaryService {
  async fetchDictionaryData(language: string): Promise<Record<string, string>> {
    try {
      const dictionary = await import(`../data/dictionary/${language}.json`);
      return dictionary.default || dictionary;
    } catch (error) {
      console.warn(
        `No dictionary data found for language: ${language}, returning empty dictionary`
      );
      return {};
    }
  }
...
export class DictionaryServiceFactory {
  ...
  create(siteName: string): DictionaryService {
    ...
    if (config.sitecoreApiKey === 'DISCONNECTED-MODE') {
      return new DisconnectedDictionaryService();
    }
